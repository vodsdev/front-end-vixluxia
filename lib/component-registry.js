...
export const AUTHORS = [
  {
    ...
    avatar: 'SF',
    bio: 'Builds polished registry components with sharp motion details.',
    links: { github: 'serafimcloud', website: 'https://21st.dev' },
  },
  {
    id: 'lina',
    name: 'Lina Moreau',
    ...
    avatar: 'LM',
    bio: 'Focuses on forms, dashboards and durable component APIs.',
    links: { github: 'linaui', website: 'https://vixluxia.com' },
  },
  {
    id: 'noah',
    name: 'Noah Studio',
    ...
    avatar: 'NS',
    bio: 'Creates animated landing sections and interaction systems.',
    links: { github: 'noahstudio', website: 'https://vixluxia.com' },
  },
  {
    id: 'maya',
    name: 'Maya Chen',
    ...
    avatar: 'MC',
    bio: 'Designs clean SaaS flows, pricing pages and conversion blocks.',
    links: { github: 'mayacodes', website: 'https://vixluxia.com' },
  },
  {
    id: 'vixluxia',
    name: 'VixLuxia Team',
...
export function enrichComponent(component, categorySlug, index = 0) {
  ...
  const publishedDaysAgo = (seed % 42) + 1;
  const votes = 24 + (seed % 920);
  const downloads = 240 + (seed % 18300);
  const comments = 2 + (seed % 48);
  ...
  return {
    ...
    meta: {
      publishedAt: new Date(Date.now() - publishedDaysAgo * 86400000).toISOString(),
      updatedAt: new Date(Date.now() - (publishedDaysAgo % 9) * 86400000).toISOString(),
      difficulty: pick(DIFFICULTIES, seed),
      license: pick(LICENSES, seed + 2),
      version: `1.${seed % 9}.${seed % 17}`,
      premium: isPremium,
      status: seed % 7 === 0 ? 'Updated' : 'Stable',
      figmaReady: seed % 3 === 0,
    },
  };
}
...
export function getAuthorsWithStats() {
  ...
  return AUTHORS.map((author) => {
    ...
    return {
      ...author,
      stats: totals,
      tags: Array.from(new Set(owned.flatMap((component) => component.tags).slice(0, 4))),
