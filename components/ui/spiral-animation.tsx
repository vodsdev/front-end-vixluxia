...
export function SpiralAnimation() {
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // Adjust the time offset slightly based on mouse to "move" the spiral
            if (animationRef.current) {
                // @ts-ignore
                animationRef.current.timeOffset = (x / rect.width) * 0.5;
                // @ts-ignore
                animationRef.current.hue = (y / rect.height) * 360;
            }
        };
    ...
    useEffect(() => {
        ...
        // Ensure hue and timeOffset exists on controller
        animationRef.current = new AnimationController(canvas, ctx, dpr, size)
        // @ts-ignore
        animationRef.current.hue = 220; // Default blue-ish
        // @ts-ignore
        animationRef.current.timeOffset = 0;
        
        // Monkey-patch render to use dynamic colors
        const originalRender = animationRef.current.render.bind(animationRef.current);
        animationRef.current.render = () => {
            // @ts-ignore
            const hue = animationRef.current.hue || 0;
            ctx.fillStyle = `hsl(${hue}, 100%, 70%)`; // use the dynamic color instead of white for stars
            originalRender();
            // Restore context fillStyle back to a dynamic color just in case
        };
