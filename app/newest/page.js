...
export default function NewestPage() {
  ...
  const allComponents = useMemo(() => {
    return getAllRegistryComponents().reverse();
  }, []);

  const newest = useMemo(() => {
    if (!search) return allComponents;
    const q = search.toLowerCase();
    return allComponents.filter(c => c.name.toLowerCase().includes(q) || c.tagline.toLowerCase().includes(q));
    ...
    <SidebarProvider>
      <div className="flex min-h-screen w-full bg-background">
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <AppHeader title="Newest" />
          <div className="flex-1 overflow-auto">
            <div className="p-6 lg:p-8 max-w-[1400px] mx-auto w-full">
              <div className="mb-8">
                <h1 className="text-2xl font-bold tracking-tight">Newest Components</h1>
                <p className="text-sm text-muted-foreground mt-1">Recently published components from the community</p>
              </div>
